...
export default function HomePage() {
  ...
  return (
    <main className="min-h-screen bg-[#FAF8F5] text-[#0D2329] selection:bg-[#2C5E50] selection:text-white">
      ...
      {/* 4. REAL LOCATION & CONNECTIVITY */}
      <LocationConnectivity />

      {/* 5. PROPOSED 3D ARCHITECTURAL EXTERIOR ELEVATION */}
      <BuildingCGIViewer onSelectFloor={handleFloorSelectFromCGI} />

      {/* 6. INTERACTIVE MASTER PLAN & CAD FLOOR EXPLORER */}
      <MasterPlanExplorer
        selectedFloorId={selectedFloor}
        onSelectUnit={handleUnitSelectFromMasterPlan}
      />

      {/* 7. 1 RK & 1 BHK RESIDENCE VISUALIZER (2D BLUEPRINT + 3D INTERIOR CGI) */}
      <ResidenceUnitExplorer initialUnitId={selectedUnit} />

      {/* 8. NOT A RETIREMENT HOME: 8 DEEP BENEFITS & COMPARISON */}
      <WhySeniorLivingBenefits />

      {/* 9. MODULAR 3-PILLAR ECOSYSTEM (HEALTHCARE, AYURVEDA, LIFESTYLE) */}
      <EcosystemShowcase />
      ...
      {/* 12. DEVELOPMENT ROADMAP (FROM VISION TO REALITY) */}
      <DevelopmentRoadmap />

      {/* 13. 64 PLOTS AVAILABILITY MATRIX & EARLY RESERVATION */}
      <AvailabilityMatrix />

      {/* 14. PROPERTY SPECIFIC FAQS */}
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center space-y-3 mb-14">
            ...
            <p className="text-xs sm:text-sm text-[#53676E] max-w-xl mx-auto">
              Clear answers regarding pre-launch booking, home loans, on-premise medical infrastructure, unit dimensions, and site visits.
            </p>
          </div>
